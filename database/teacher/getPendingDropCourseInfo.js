...
const getPendingDropCourseInfo = async (userId) => {
    try {
        console.log(userId);
        const query = `
        SELECT 
        ...
        const results = await new Promise((resolve, reject) => {
            pool.query(query, [userId, 'waiting for delete'], (error, results) => {
                if (error) {
        ...
        return { success: true, message: 'pending drop course list', results };
    } catch (error) {
...
module.exports = { getPendingDropCourseInfo };