...
const teacherMarkAssignFunction = async (data) => {
    try {
        ...
        const results2 = await new Promise((resolve, reject) => {
            pool.query(courseIdQuery, courseTitle, (error, results) => { // pass query directly, don't call the function
                if (error) {
                    console.error('Error executing query:', error);
                    reject(error);
                } else {
        ...
        `;
        const queryValues = results3.map(row => [row.TEACHER_SERIAL_NO, courseId]);
